...
verify_cluster_health() {
    ...
    local tries=0; while [[ $tries -lt 12 ]]; do local bad; bad=$(kubectl get pods -n kube-system --no-headers 2>/dev/null | grep -v "Running\|Completed" | wc -l || true); [[ "$bad" -eq 0 ]] && break; ((tries++)); sleep 10; done; ok "System pods healthy"
}
