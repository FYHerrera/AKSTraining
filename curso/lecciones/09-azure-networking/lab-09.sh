...
deploy() {
    ...
    echo -e "    • A web pod with a LoadBalancer Service (web-public)"
    [[ -n "$ext_ip" ]] && [[ "$ext_ip" != "<pending>" ]] && echo -e "    • External IP: ${CYAN}$ext_ip${NC}"
    echo ""
    echo -e "  ${RED}${BOLD}What's wrong:${NC}"
    echo -e "    • HTTP requests to the External IP are being BLOCKED"
    echo -e "    • The Service has an IP but port 80 traffic never reaches the pod"
    echo ""
    echo -e "  ${GREEN}${BOLD}Your task:${NC}"
    echo -e "    • Find what's blocking port 80 at the Azure networking level"
    echo -e "    • MC Resource Group: ${CYAN}$MC_RESOURCE_GROUP${NC}"
    echo -e "    • Hint: ${CYAN}az network nsg list -g $MC_RESOURCE_GROUP${NC}"
}
