...
EOF
    ...
    info "PVC: db-storage (5Gi, ReadWriteOnce)"
    info "  StorageClass requested: premium-ssd-nonexistent"
    info "Pod: database — mounts db-storage at /var/lib/data"

    echo ""; separator
    header "What's wrong"
    err "PVC 'db-storage' is stuck in Pending!"
    err "The pod can't start because the volume isn't bound."

    echo ""; separator
    header "Your task"
    info "Find out why the PVC is Pending and fix it."
    info "Start with: kubectl get pvc"
    info "Also check: kubectl get storageclass"
}
