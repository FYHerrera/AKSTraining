#!/usr/bin/env bash
###############################################################################
# Lab 02 – Fix ImagePullBackOff  (STANDALONE)
#
# Scenario : A deployment references a non-existent image tag causing
#            ImagePullBackOff. Diagnose using describe and fix the image.
# Objective: Get all 3 replicas running with a valid image.
#
# Usage:  chmod +x lab-02.sh && ./lab-02.sh
###############################################################################
...
check_prerequisites() {
    ...
    if [[ ${#missing[@]} -gt 0 ]]; then
        ...
        if [[ "${ans,,}" =~ ^y ]]; then
            ...
            for p in "${missing[@]}"; do local w=0; while [[ "$(az provider show --namespace "$p" --query 'registrationState' -o tsv 2>/dev/null)" != "Registered" ]]; do ((w++)); [[ $w -ge 30 ]] && { err "Timeout for $p"; exit 1; }; sleep 10; done; ok "$p registered"; done
...
cleanup_resources() {
    ...
    if [[ "${response,,}" =~ ^y ]]; then log "Deleting resource group $RESOURCE_GROUP..."; az group delete --name "$RESOURCE_GROUP" --yes --no-wait 2>/dev/null || true; ok "Deletion initiated."
...
###############################################################################
# Lab 02 – Fix ImagePullBackOff
###############################################################################
LAB_NAME="pods-containers"
LAB_TITLE="Lab 02 – Fix ImagePullBackOff"
LAB_DESC="
  ${BOLD}Scenario${NC}
  A developer pushed a deployment with an incorrect image tag.
  All pods are stuck in ${RED}ImagePullBackOff${NC}.

  ${BOLD}Objective${NC}
  Fix the deployment ${CYAN}web-app${NC} so all 3 replicas are ${GREEN}Running${NC}.

  ${BOLD}Useful commands${NC}
    kubectl get pods
    kubectl describe pod <name>
    kubectl get deployment web-app -o yaml
    kubectl set image deployment/web-app <container>=<image:tag>
"

deploy() {
    create_aks_cluster "$LAB_NAME"
    header "Injecting Lab Scenario"
    log "Deploying broken web-app..."
    kubectl apply -f - <<'EOF'
apiVersion: apps/v1
kind: Deployment
metadata:
...
validate() {
    ...
    [[ "$running" -ge 3 ]] && { ok "All 3 replicas are Running!"; return 0; } || { err "$running/3 replicas running."; return 1; }
}

hint() {
    local attempt="${1:-0}"; echo ""
    if [[ $attempt -lt 2 ]]; then info "Hint 1: Check Events in 'kubectl describe pod <name>'"; info "  Look for the error about pulling the image."
    elif [[ $attempt -lt 4 ]]; then info "Hint 2: The image tag 'nginx:99.99.99-nonexistent' doesn't exist."; info "  Set a valid tag like nginx:1.25"
    else info "Hint 3: kubectl set image deployment/web-app nginx=nginx:1.25"; fi
}

solution() {
    echo ""; header "Solution"
    info "The image tag is invalid. Fix it with:"
    echo -e "  ${CYAN}kubectl set image deployment/web-app nginx=nginx:1.25${NC}"; echo ""
    info "Or edit the deployment:"; echo -e "  ${CYAN}kubectl edit deployment web-app${NC}"
    info "Change 'nginx:99.99.99-nonexistent' to 'nginx:1.25'"
}
